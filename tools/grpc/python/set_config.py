...
def run():
    ...
    try:
        ...
        try:
            reply = stub1.SetConfig(gobgp_pb2.SetConfigRequest(config=config1, config_format=gobgp_pb2.TOML))
        ...
        try:
            reply = stub2.SetConfig(gobgp_pb2.SetConfigRequest(config=config2, config_format=gobgp_pb2.TOML))
        except grpc.RpcError as e:
            print e.details()
            status_code = e.code()
            print status_code.name
        ...
        try:
            reply = stub2.SetConfig(gobgp_pb2.SetConfigRequest(config=config3, config_format=gobgp_pb2.TOML))
        except grpc.RpcError as e:
            print "Received expected Exception setting bad config"
            print e.details()
            status_code = e.code()
            print status_code.name
            print status_code.value
